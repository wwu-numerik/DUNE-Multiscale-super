...
public:
    ...
    void init(MPI_Comm comm, const COR& corrFunction, int directionSz, int seed, int overlap=1, TypeOfGen genType = TypeOfGen::CellCenter, double minimal=1e-8)
    {
        ...
        this->localBuffsz = new int[commSz];
        this->dispArr = new int[commSz];

        this->comm = comm;
        this->corr_functor = corrFunction;
        this->seed = seed;
        this->minimal = minimal;
        this->typeOfGen = genType;

        //Set seed and random engine
        this->rand = std::default_random_engine(seed);
        this->normal = std::normal_distribution<double>(0,1);

        this->base_size = (1 << (directionSz + genType));
        this->segment_size = (this->base_size << 1);        // Extend FFT matrix
        this->step = 1.0 / base_size;

        // Alloc memory
        this->alloc_local = fftw_mpi_local_size_2d(segment_size, segment_size, this->comm, &local_size,  &local_start);
        this->p_base = fftw_alloc_complex(alloc_local);
        this->p_layer = fftw_alloc_complex(alloc_local);
        this->permeabilityField.resize(segment_size * segment_size);

        // Populate boundries of permability, should be one call
        ...
        //Create plans
        this->fft_plan = fftw_mpi_plan_dft_2d(segment_size, segment_size, this->p_base, this->p_base, this->comm, FFTW_FORWARD, FFTW_ESTIMATE | FFTW_MPI_TRANSPOSED_OUT);
        this->ifft_plan = fftw_mpi_plan_dft_2d(segment_size, segment_size, this->p_layer, this->p_layer, this->comm, FFTW_BACKWARD, FFTW_ESTIMATE | FFTW_MPI_TRANSPOSED_IN);
        ...
        fftw_execute(this->ifft_plan);
        this->generateLocalPerm();
        this->populate();

        timeForGeneration = MPI_Wtime() - timeForGeneration;

        this->leftPermFields = 1;
        this->permFieldIndex = 0;
    }
    ...
    R operator() (const X& x) const
    {
        ...
        if (floatCompare<double>(tmp[0], 1.0, 1e-15)) {
            tmp[0] -= step;
        }
        if (floatCompare<double>(tmp[1], 1.0, 1e-15)) {
            tmp[1] -= step;
        }
