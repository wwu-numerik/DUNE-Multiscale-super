...
def update(branch, cc):
    ...
    dockerfile = os.path.join(os.path.dirname(__file__), 'dune-multiscale-testing.docker')

    os.chdir(gdt_super_dir)

    cxx = cc_mapping[cc]
    subprocess.check_call(['docker', 'build', '-f', '.travis/docker/dune-multiscale-testing.docker',
                        '-t', 'dunecommunity/dune-multiscale-testing:{}_{}'.format(cc, branch), '--build-arg', 'cc={}'.format(cc),
